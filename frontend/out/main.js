    ...
    onload(d) {
        ...
        b_submit.onclick = async function () {
            ...
            socket.emit("uploadSS", fileRef.name, i_w.value, height, width, height, i_title.value, i_desc.value, {
                speed: i_speed.value
            }, await fileRef.arrayBuffer(), async (fileId) => {
                if (fileId == null) {
...
function getFilePropAsync(fileId, prop) {
    return new Promise(resolve => {
        socket.emit("getFileProps", fileId, prop, (data) => {
            resolve(data);
        });
    });
}
...
async function loadFile(s, cont) {
    s = s.toString();
    let file = [];
    let amt = await getFilePropAsync(s, "sect");
    for (let j = 0; j < amt; j++) {
        file[j] = await new Promise(resolve => {
    ...
    if (id == "personal") {
        ...
        async function loadSect(txt, type, atr = null) {
            ...
            car.onclick = async function () {
                ...
                if (car.open) {
                    //loadFile
                    async function loadFileSect(filesStr) {
                    ...
                    if (type == "allFiles") {
                        ...
                        let filesStr = await getPropAsync(username, "likes");
                        await loadFileSect(filesStr);
...
async function addFile(file, par = null, scale = 1) {
